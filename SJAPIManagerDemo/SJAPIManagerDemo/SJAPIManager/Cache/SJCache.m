...
- (NSCache *)cache{
    if (nil == _cache) {
        ...
        _cache.countLimit = kSJSCacheCountLimit;
    }
    return _cache;
}
...
- (NSData *)fetchCachedDataWithKey:(NSString *)key{
    SJCachedObject *cachedObject = [self.cache objectForKey:key];
...
- (void)saveCacheWithData:(NSData *)cachedData key:(NSString *)key
{
    SJCachedObject *cachedObject = [self.cache objectForKey:key];
    if (cachedObject == nil) {
        cachedObject = [[SJCachedObject alloc] init];
    }
...
//        matchKey = [NSString stringWithFormat:@".*_%@_%@.*", SBAPPDelegate.token ? SBAPPDelegate.token : @"token", regexKey];
        ...
        for (NSString* key in [self.keys allObjects]) {
            if ([key matchWithRegex: matchKey]) {
                [self deleteCacheWithKey:key];
            }
        }
    }
...
- (NSString*)keyWithAPIResources:(NSString *)methodName requestParams:(NSDictionary *)requestParams{
    return [NSString stringWithFormat:@"%@%@",methodName, [requestParams SJSUrlParamsToStringSignature:NO]];
}
